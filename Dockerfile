...
RUN sudo apt-get install ca-certificates curl gettext tar gzip \
    ...
    && wget https://storage.googleapis.com/kubernetes-helm/${HELM_FILENAME} && tar xfz $HELM_FILENAME && sudo mv linux-amd64/helm /usr/local/bin/helm && sudo chmod +x /usr/local/bin/helm && rm -rf ${HELM_FILENAME} linux-amd64\ 
    && helm version ||true
